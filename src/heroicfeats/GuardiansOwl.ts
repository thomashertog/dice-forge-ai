...
export class GuardiansOwl extends AbstractHeroicFeatCard implements ReinforcementEffect{
    ...
    async handleReinforcement(game: Game, currentPlayer: Player): Promise<boolean> {
        ...
        let answer = await currentPlayer.getUserInterface().chooseResource(game, currentPlayer.name, value, 'G', 'M', 'S', 'C');
        
        switch(answer){
            case 'G': const goldForHammer = await currentPlayer.getUserInterface().howMuchGoldForHammer(game, currentPlayer, value);
